...
| NTP 減少伺服器數量後清除舊 slot | PH_SCALANCE-S615-CLI_76 sec 7.2.3.2 p. 218（`no ntp server id <1-3>`） | **Fixed 2026-04**。`SetNtpAsync` 先前只覆寫前 N 個 slot，從 3 台降為 1 台時 id=2/3 的舊伺服器會殘留。現在在覆寫後對未使用的 slot 逐一 emit `no ntp server id M`（idempotent，對空 slot 為 no-op）。|
| `interface` 名稱自動正規化（`vlan1` → `vlan 1`、`ppp2` → `ppp 2`） | PH_SCALANCE-S615-CLI_76 範例 p. 65 / p. 430 使用 `int vlan 1`（有空格）；sec 12.3.4.1 p. 598-599 iftype 表列 `vlan` 與 `ppp` 為需要 ifstring 的 iftype | **Fixed 2026-04**。新增 `NormalizeInterfaceName` pure 方法並在 `BuildSetInterface` 自動呼叫。WBM 輸出或某些韌體的 `show ip interface` 會回傳緊湊形式 `vlan1`，原本 load→edit→apply 流程會把它照抄送出 `interface vlan1` 被裝置拒絕；現在自動轉換。未知前綴直接透傳（`Device`、`fa 0/1` 等）。|
| `RunBatchAsync` 跨指令模式狀態（critical） | SCALANCE CLI mode 切換（`configure terminal` → `ntp` → `events` → `ipsec` → …）依賴同一 session 中後續指令共享狀態 | **Fixed 2026-04**。原實作 `foreach RunAsync` 為每條指令開一條獨立 SSH exec channel，SCALANCE 在每次 exec 間重置回 Privileged EXEC，造成 `configure terminal` 後 `ntp` 永遠不會進入 NTP config mode — 所有寫入 batch 理論上都應該失敗。改為把整批指令以 `\n` 串接成一條 exec body，由裝置在同一 CLI input buffer 內序列處理。保留 `IReadOnlyList<string>` 簽章（回傳單一元素含合併輸出）。|
...
| Ping IPv6 literal 走專用 `ping ipv6` 指令 | PH_SCALANCE-S615-CLI_76 sec 5.1.9 p. 87（IPv4 的 `ping` 與 IPv6 的 `ping ipv6` 為兩條獨立指令，選項順序也不同） | **Fixed 2026-04**。先前 `FormatPingCommand` 把 IPv6 literal（含冒號）誤判為 FQDN 而發出 `ping fqdn-name 2001:db8::1`，裝置會拒絕。改為 IPv6 literal 直接走 `ping ipv6 <addr>`，選項順序比照手冊 p.87（count → size → timeout）；IPv4 分支維持 p.86 的 size → count → timeout 順序。|
...
| SNMP 代理硬化（enabled / version / port / port reset） | PH_SCALANCE-S615-CLI_76 sec 9.8.2.1-2 pp. 437-438（`snmpagent` / `no snmpagent`）；sec 9.8.2.5 pp. 441-442（`snmp agent version {v3only\|all}`）；sec 9.8.2.16 p. 451（`snmpagent port <1024-65535>`）；sec 9.8.2.17 p. 452（`no snmpagent port` 重設為 161） | **Added 2026-04 + extended**。新增 `SnmpAgentVersionPolicy` enum + `SnmpAgentConfig` 模型、四個 builders、四個 driver 方法、「SNMP 代理」分頁（啟用 checkbox、僅允許 SNMPv3 硬化 checkbox、監聽埠輸入框各自獨立 Apply 按鈕、**「重設為 161」按鈕**走 `no snmpagent port`，因為可設定範圍 1024-65535 不含預設值）。|
...
| 排程重啟 `schedule restart-timer <sec(300-86400)> [force]` + `cancel restart-timer` | PH_SCALANCE-S615-CLI_76 sec 5.3.2.3-4 pp. 133-134 | **Added 2026-04**。`BuildScheduleRestartTimer(seconds, force=true)`（強制 300-86400 秒 = 5 分-24 小時範圍、`force` 關鍵字讓 headless script 可用）、`BuildCancelRestartTimer()`；`IDeviceDriver.ScheduleRestartAsync` / `CancelScheduledRestartAsync`；「診斷」分頁在 restart 區塊下方加入排程輸入與兩顆按鈕。|
...
## 2026-05-04 實機 write/read/cleanup probes(`tools/probe_*.py`)

5 條 paramiko-based probe 對 192.168.1.1 V08 實機跑完(全部 cleanup 後回 pre-state)。每個 probe 都做 snapshot → write → read-back → cleanup → verify。Probes:`probe_firewall_ipv4rule.py` / `probe_syslog_dns.py` / `probe_ntp_server.py` / `probe_snmp_toggle.py` / `probe_subnet_alias_tia.py`。

| 項目 | 來源 | 狀態 |
|---|---|---|
| **Firewall `ipv4rule from vlan 1 to vlan 2 srcip 0.0.0.0/0 dstip 0.0.0.0/0 action acc service all comment <X>`** | `probe_firewall_ipv4rule.py` 對真機,Set+Read+Cleanup PASS | **Verified live 2026-05-04**。`from vlan 1`(空格)語法被裝置接受;action `acc` 在 `show firewall ip-rules ipv4` 回讀為 `ACCEPT`(parser 要做 `acc`↔`ACCEPT` 映射);`comment` 字串原樣保留;`no ipv4rule idx N` 乾淨刪除。`BuildCreateFirewallRule` / `BuildDeleteFirewallRule` 可放心用。|
| **`syslogserver ipv4 …` 條目落在 `show running-config syslog all`,**不是** `events all`** | `probe_syslog_dns.py` 第一輪只查 events 顯示 FAIL,加查 syslog 後 PASS | **Found 2026-05-04**。雖然 `BuildAddSyslogServer` 進入 `events` mode 才送 `syslogserver` 命令,V08 把它寫入 `syslog` config section;`show running-config events all` 看不到(只有 `client config syslog` / `event config X syslog` 等 dispatch 設定)。`SyslogServer` load 流程必須 query `show running-config syslog all`(或 dedicated `show syslog-server` 若有)而非 events。Probe 已修為兩段都查取 OR。|
| **NTP `poll <sec>` 不 round-trip;寫第一台 server 會自動長 secure 配對** | `probe_ntp_server.py` 對真機 V08 | **Found 2026-05-04**。送 `ntp server id 1 ipv4 1.1.1.1 poll 64` 整行被裝置接受(無 `% Invalid`),但 `show running-config ntp all` 只顯示 `ntp server id 1 ipv4 1.1.1.1` — `poll 64` 沒持久化(可能等於預設值被省略,或 V08 silent-drop)。額外:寫第一台 NTP server 時裝置**自動加** `ntp server id 1 secure ntpkeyid 1 hashalg des-cbc` 一行,即使前面 `no ntp secure` 仍在(secure-ntp 整體仍 disabled,但 running-config 多一行)。`ParseNtp` 與 round-trip 邏輯要過濾這條 secure-pair 才不會誤回填。`BuildSetNtp` 不擴 `poll`(無價值);若需要 poll 行為驗證走 `show ntp info` 而非 running-config。|
| **DNS `manual srv <ip>` 與 `server type all` 共存** | `probe_syslog_dns.py` 對真機 V08 | **Verified live 2026-05-04**。新增 `manual srv 8.8.8.8` 不需先切 `server type manual`,`show running-config dnsclient all` 同時顯示 `server type all` + `manual srv 8.8.8.8`。`no manual srv 8.8.8.8` 在 `dnsclient` mode 下被接受並乾淨刪除。手冊未明寫 `no manual srv` 但語法慣例成立 — 可放心對外暴露 RemoveDns 流程。|
| **SNMP agent toggle:`show running-config snmp all` 用 last-write-wins 編碼** | `probe_snmp_toggle.py` 對真機 V08(disable→re-enable→verify) | **Verified live 2026-05-04**。Pre-state 同時有 `no snmpagent` + `no snmpagent port` + `snmpagent` 三行(後者覆蓋前者 = enabled);disabled 後 `snmpagent` 那行消失,只剩 `no snmpagent`。Parser 必須掃 LAST occurrence 取 effective state,不能用 first-match 或 grep `^snmpagent$` 否則 disabled 也會誤判 enabled。Probe disable 階段刻意**不**送 `write startup-config` — 萬一 mid-probe 斷電,裝置開機回 enabled 是 desirable behavior;只有 re-enable 階段才 commit。|
| **`BuildSetInterface` 的 `no ip address` 在 management VLAN 上有 SSH 中斷風險** | `probe_subnet_alias_tia.py` 對真機 V08 vlan1(192.168.1.1/24,SSH 來源同網段) | **Found 2026-05-04(coverage gap)**。當 IP 與 SSH 來源同 VLAN 時,builder 的 `no ip address` 會把 management interface IP 移除,可能在後續 `ip address X` 生效前讓 SSH session 斷線。Probe 因此**故意省略** `no ip address` 步驟,只測 idempotent re-apply(alias INT + tia-interface + 同值 ip address):Set/Read/Persist PASS。**遺留:DHCP→static 切換路徑(那才需要 `no ip address`)未在實機上驗證**;App Apply 在 management VLAN 切換 DHCP↔static 應走 console / WBM,或在 SshSession 內把整個批次以單一 chan.send 一次推出讓裝置在斷線前處理完整批(`SshSession.RunBatchAsync` 已是這個行為,實質風險可能比想像中小,但需要 lab 機器或 vlan2 驗證才能放心 flip default)。|
| **vlan1 stanza 在 `show running-config interface vlan 1 all` 同時包含 `alias INT` + `tia-interface` + `ip address` + IPv6 ND 預設組** | `probe_subnet_alias_tia.py` snapshot 與 read-back 對比 | **Verified live 2026-05-04**。完整 stanza:`alias INT` / `tia-interface` / `no shutdown` / `no ip address dhcp` / `ip address 192.168.1.1 255.255.255.0` / `ipv6 nd suppress-ra` / `no ipv6 nd other-config flag` / `no ipv6 nd managed-config flag` / `ipv6 nd ra-lifetime 1800` / `ipv6 nd reachable-time msec 30000` / `ipv6 nd mtu 1500` / `ipv6 address autoconfig`。`ParseInterfaces` 已知 `alias` 與 `tia-interface`;若 round-trip 要保留 IPv6 ND 設定,需要新增 `Ipv6NdConfig` 欄位族(目前 builder 略過 IPv6 ND,read-modify-write 會丟失非預設值)。|
